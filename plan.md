...
- Note: runs are reproducible when `Model(seed=...)` is given (all randomness is drawn from a per-model generator, `model.rng`); without a seed they still differ run to run. Seeding is irrelevant to pause/resume itself, but it is what a future "replay to reconstruct state" approach would build on.
